...
lister_meta_path <- '/lustre/orion/syb111/proj-shared/Projects/scrna-seq/data/human/brain/healthy/lister_lab/meta/cell_meta_imputed_final_labels.tsv'


split_by_celltype <- function(obj, out_prefix) {
    #split_obj <- SplitObject(obj, split.by='Granular.cell.type')
    metadata <- read.table(lister_meta_path, sep='\t')
...
for (obj_path in obj_paths) {
    ...
    if (!grepl('lister', obj_path, fixed=TRUE)) { next }
