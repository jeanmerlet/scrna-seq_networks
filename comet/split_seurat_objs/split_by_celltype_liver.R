...
celltype_colname <- 'annot'
#obj_path <- '/lustre/orion/syb111/proj-shared/Projects/scrna-seq/data/human/liver/healthy/vcir/seurat/all_liver.rds'
obj_path <- '/lustre/orion/syb111/proj-shared/Projects/scrna-seq/data/human/liver/healthy/vcir/seurat/all_liver_imputed.rds'
...
tenx_to_seurat <- function(obj) {
    metadata <- read.table(meta_path, sep=',', header=TRUE, row.names=7)
    print(str(metadata))
    obj <- CreateSeuratObject(obj)
    obj <- AddMetaData(obj, metadata)
    # change subset expression as needed
    obj <- subset(obj, subset = typeSample == 'scRnaSeq')
    print(str(obj))
    return(obj)
}
