...
choose_k <- function (mat, K=100, thresh=6, noise_start=80, q=2, use.mkl=F, mkl.seed=-1) {
    ...
    # 
    # Args:
    #   mat: The log-transformed expression matrix of cells (rows) vs. genes (columns)
    ...
    #       3) Singular values of the matrix mat
    ...
    if (!use.mkl) {
        rsvd_out <- randomized.svd(mat, K, q=q)
    }else {
        rsvd_out <- randomized.svd(mat, K, q=q, method='rsvd-mkl', mkl.seed=mkl.seed)
    }
...
alra <- function(mat, k=0, q=10, quantile.prob=0.001, use.mkl=F, mkl.seed=-1) {
    # Computes the k-rank approximation to mat and adjusts it according to the
    ...
    # Returns:
    #   A list with three items
    #       1) The rank k approximation of mat.
    #       2) The rank k approximation of mat, adaptively thresholded
    #       3) The rank k approximation of mat, adaptively thresholded and
    #       with the first two moments of the non-zero values matched to the
    #       first two moments of the non-zeros of mat. This is the completed
    #       matrix most people will want to work with
    # Example:
    #     result.completed <- adjusted_svd(mat,15)
    #     mat_rank15 <- result.completed[[1]]     # The low rank approximation for reference purposes...not suggested for matrix completion
    #     mat_rank15_cor <- result.completed[[3]] # The actual adjusted, completed matrix

    cat(sprintf('Read matrix with %d cells and %d genes\n', nrow(mat), ncol(mat)))

    if (!(inherits(mat, 'matrix'))) {
        stop(sprintf('mat is of class %s, but it should be of class matrix. Did you forget to run as.matrix()?',class(mat)))
    }

    if (k == 0) {
        k_choice <- choose_k(mat)
        k <- k_choice$k
    ...
    cat('Randomized SVD\n')
    if (!use.mkl) {
        fastDecomp_noc <- randomized.svd(mat, k, q=q)
    } else {
        fastDecomp_noc <- randomized.svd(mat, k, q=q, method='rsvd-mkl', mkl.seed=mkl.seed)
    }
    mat_rank_k <- fastDecomp_noc$u[,1:k] %*% diag(fastDecomp_noc$d[1:k]) %*% t(fastDecomp_noc$v[,1:k])
...
mat_rank_k_cor_sc[mat_rank_k_cor==0] = 0
    ...
    #mat_rank_k_temp <- sweep(mat_rank_k_temp,2, toadd[toscale],FUN = '+')

    #mat_rank_k_cor_sc <- mat_rank_k_cor
    #mat_rank_k_cor_sc[,toscale] <- mat_rank_k_temp
    #mat_rank_k_cor_sc[mat_rank_k_cor==0] = 0
    ...
    # set originally nonzero values that are now 0 from thresholding back to their pre-imputation nonzero values
    mat_rank_k_cor_sc[originally_nonzero & mat_rank_k_cor_sc == 0] <- mat[originally_nonzero & mat_rank_k_cor_sc == 0]

    colnames(mat_rank_k_cor) <- colnames(mat)
    colnames(mat_rank_k_cor_sc) <- colnames(mat)
    colnames(mat_rank_k) <- colnames(mat)
    ...
    #return (list(mat_rank_k=mat_rank_k, mat_rank_k_cor=mat_rank_k_cor, mat_rank_k_cor_sc=mat_rank_k_cor_sc))
