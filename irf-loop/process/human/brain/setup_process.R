...
code_dir <- "/lustre/orion/syb111/proj-shared/Projects/scrna-seq/code/irf-loop/process/human/brain/"
log_dir <- "/lustre/orion/syb111/proj-shared/Projects/scrna-seq/code/irf-loop/process/human/brain/logs/"
...
lapply(1:length(irf_matrices),function(cell_type) {
    ...
    tryCatch({
        num_cells <- countLines(irf_matrix_cell_type)[1] 
        if(num_cells > 0 & num_cells  < 1000) {
            time <- "2:00:00"
        } else if(num_cells >= 1000 & num_cells < 12000) {
            time <- "6:00:00"
        } else {
            time <- "12:00:00"
        }
        ...
        error = function(x) {
	    print("cell type doesn't exist for region")
	}
    )
})
